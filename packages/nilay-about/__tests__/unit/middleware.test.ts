...
describe('production Content Security Policy', () => {
  ...
  it.each(['http://localhost:3001', 'http://127.0.0.1:3001', 'http://[::1]:3001'])(
    ...
    async (origin) => {
      ...
      const response = middleware(new NextRequest(`${origin}/contact`, { headers: { host: new URL(origin).host } }));
  ...
  it.each([
    ...
    async (headers) => {
      const { middleware } = await import('@/middleware');
      const response = middleware(new NextRequest('http://127.0.0.1:3001/contact', { headers }));
      expect(response.headers.get('Content-Security-Policy')).toContain('upgrade-insecure-requests');
    },
  );
});
