...
test.describe('Home Target Calculator', () => {
  ...
  test('should display calculator interface', async ({ page }) => {
    ...
    await expect(page.getByText('標的の中心の高さ', { exact: true })).toBeVisible();
    await expect(page.getByText('黒い領域の大きさ', { exact: true })).toBeVisible();
  ...
  test('should calculate target height dynamically', async ({ page }) => {
    ...
    // Wait for the recalculated output to render after the input event.
  ...
  test('should switch language to English', async ({ page }) => {
    ...
    await expect(page.getByText('Height of Target Center', { exact: true })).toBeVisible();
    await expect(page.getByText('Black Area Size', { exact: true })).toBeVisible();
  });
  ...
  test('should enable custom discipline mode', async ({ page }) => {
    ...
    const distanceInput = page.getByRole('dialog').getByRole('spinbutton').first();
