...
export function ContactForm() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    try {
      ...
      showSuccess('送信完了', `お問い合わせありがとうございます。— お問い合わせ番号：${result.uuid}`);
      reset();
    } catch {
      showError(
        '送信エラー',
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-4">
      <p>
        <input type="checkbox" id="requiresReply" disabled={isLoading} {...register('requiresReply')} />{' '}
      ...
      {alert && (
        <p className={alert.type === 'success' ? 'text-foreground' : 'text-destructive'}>
