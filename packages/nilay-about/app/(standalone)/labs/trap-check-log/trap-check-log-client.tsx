...
import { LuDownload, LuPlus, LuPrinter, LuTrash2 } from 'react-icons/lu';
...
export function TrapCheckLogClient() {
  const { intervalHours, lastValidIntervalHours, traps, setIntervalHours, resetInterval, addTrap, clearAll } =
  ...
  const setDraftField = (field: keyof TrapDraft, value: string) => setDraft((state) => ({ ...state, [field]: value }));
  ...
  const exportCsv = () => {
    ...
    const blob = new Blob([`﻿${buildTrapCheckCsv(traps, language)}`], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `trap-check-log-${currentLocalMinute().slice(0, 10)}.csv`;
        ...
        <ToolLayout
            ...
            <section aria-labelledby="traps-heading" className="space-y-4">
              ...
              {traps.length === 0 ? (
                <p className="rounded-sm bg-surface-container p-4 text-sm">
                  {t(
                    'まだわなが登録されていません。設置したら下のフォームで登録してください。',
                    'No traps yet. Register each trap below when you set it.',
                  )}
                </p>
              ) : (
                <>
                  <p className="text-sm text-on-surface-variant">
                    {t('見回りを待っている時間が長い順。撤去済みは最後。', 'Longest wait first. Removed traps last.')}
                  </p>
                  {ordered.map((trap) => (
                    <TrapCard
                      key={trap.id}
              ...
              <Card variant="outlined" className="space-y-4 rounded-md p-5 sm:p-6">
                ...
                <fieldset className="space-y-2">
                  ...
                  <p className="text-xs text-on-surface-variant">
                    {t(
                      '手入力のみ。位置情報の取得や外部への送信はしません。',
                      'Typed in by hand. Your location is not read or sent anywhere.',
                    )}
                  </p>
                </fieldset>
          ...
          result={
            <Card variant="outlined" className="min-w-0 space-y-5 rounded-md p-5 sm:p-6">
              ...
              <NumberField
                ...
                hint={t(
                  `最後の見回り（なければ設置）からこの時間を過ぎると警告します。初期値の ${DEFAULT_INTERVAL_HOURS} 時間は「毎日」の案内に合わせたもので、法令の定めではありません。許可の条件があればそれに合わせてください。`,
                  `A trap is flagged once this time has passed since its last round (or since it was set). The default ${DEFAULT_INTERVAL_HOURS} hours follows the “every day” guidance and is not set by law. Follow your permit if it sets an interval.`,
                )}
              />
              {traps.length > 0 && (
              ...
              <p className="text-xs text-on-surface-variant">
                {t(
                  '通知はしません。見回りの前にこのページで確認してください。経過時間はこの端末の時計で計算します。',
                  'There are no notifications; check this page before each round. Elapsed times use this device’s clock.',
                )}
              </p>
            </Card>
          }
          secondary={
            <ConditionSection
              ...
              summary={
                storageAvailable
                  ? t('記録はこのブラウザーにだけ保存します。', 'Saved in this browser only.')
                  : t('このブラウザーでは保存できません。', 'This browser cannot save.')
              }
            >
              <p className="text-sm text-on-surface-variant">
                {t(
                  '外部には送信しません。ブラウザーのデータを消すと記録も消えます。控えは CSV に書き出してください。',
                  'Nothing is sent anywhere. Clearing browser data deletes the log; export a CSV to keep a copy.',
                )}
              </p>
          ...
          extras={
            <ConditionSection
              ...
              summary={t(
                '国の指針は「頻繁に」、県の案内は「原則として毎日」。間隔を時間で一律に定めた法令の規定は見当たりません。許可の条件と都道府県の案内に従ってください。',
                'The national guideline says “frequently”; prefectural guidance says every day as a rule. No nationwide legal provision setting a fixed check interval was found. Follow your permit conditions and prefectural guidance.',
              )}
            >
              <div className="space-y-4 text-sm">
                {language === 'en' && (
                  <p className="text-on-surface-variant">Passages are quoted in the Japanese original.</p>
                )}
                <section className="space-y-2">
                  ...
                  >
                    ...
                    <p lang={language}>
                      {t(
                        '（国と都道府県が取り組む事項で、間隔を時間で定めたものではありません。）',
                        '(What the national and prefectural governments work towards; it sets no interval in hours.)',
                      )}
                    </p>
                    <p>
                      Ⅲ第四 1(2)
                ...
                <ul className="list-disc space-y-2 pl-5 text-on-surface-variant">
                  <li>
                    {t(
                      '「毎日」は暦の日ごとの見回りで、24 時間ちょうどの間隔とは違います。間隔は運用に合わせて設定してください。',
                      '“Every day” means once per calendar day, not exactly every 24 hours. Set the interval to suit how you work.',
                    )}
                  </li>
                  <li>
                    {t(
                      '許可捕獲では、見回りの実施方法が許可の条件になることがあります。条件の内容は許可をした都道府県・市町村に確認してください。',
                      'Under a permit, how rounds are made can be a permit condition. Ask the prefecture or municipality that issued it.',
                    )}
                  </li>
                  <li>
                    {t(
                      'この記録は、法令や許可条件を満たしていることを示すものではありません。',
                      'This log does not show that a law or permit condition has been met.',
                    )}
                  </li>
                  <li>
                    {t(
                      `確認日：${CHECKED_ON}。基本指針は改正準備中のため、最新版を確認してください。`,
                      `Checked on ${CHECKED_ON}. The guideline is being revised; check the latest version.`,
                    )}
                  </li>
                </ul>
              </div>
