...
export function AccessibleImage({ alt, decorative = false, className, ...props }: AccessibleImageProps) {
  // For decorative images, use empty alt and aria-hidden
  if (decorative) {
    return <Image alt="" aria-hidden="true" className={cn('select-none', className)} {...props} />;
  }

  // Ensure alt is not empty for non-decorative images
  if (!alt.trim()) {
    console.warn('AccessibleImage: Non-decorative images must have meaningful alt text');
  }
 ...
 */
interface AvatarImageProps extends Omit<AccessibleImageProps, 'width' | 'height' | 'alt'> {
  size?: number;
  name: string;
}

export function AvatarImage({ name, size = 40, className, ...props }: AvatarImageProps) {
  return (
    <AccessibleImage
      alt={`${name}のアバター`}
      width={size}
...
export function Thumbnail({ aspectRatio = 'video', className, ...props }: ThumbnailProps) {
  return (
