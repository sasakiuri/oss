...
`hardware/shot` はメーカーの生フレームそのものではありません。Saika が変換した座標・得点を配信します。
...
大会の結果は、着弾イベントを受信するたびに確定するものではありません。Director の終了処理では最終スコアを検証し、大会の保存先へ反映します。
...
| `activate-safety-stop` / `clear-safety-stop`                     | 競技に依存せず停止。明示安全確認で解除しても、タイマーは再開しない           |
| `start-qualification-recovery` / `cancel-qualification-recovery` | 許可された追加試射・再射・補射を別の記録単位で実行                           |
| `apply-qualification-recovery` / `settle-qualification-recovery` | 復旧射の採点反映、または記録済みシリーズの維持を明示適用                     |
...
本文は外部連携に必要な意味と動作を説明しています。任意項目や相互制約を含む完全な入力形式は、利用する版の共通契約で確認できます。
