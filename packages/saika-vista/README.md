...
4. Use **Identify** to locate the physical screen. Check that its settings were saved and that the screen reports it is displaying. Offline or disconnected monitors show that their display cannot be confirmed.
...
Vista saves settings, pairing and selected display data in `vista.json` in the application's data directory. Back up this directory with Vista stopped; it contains pairing secrets and athlete data. A failed save keeps the preceding file. An unsupported file version, invalid settings or unreadable JSON prevents startup and leaves the file intact.
