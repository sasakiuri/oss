...
const userDataPath = join(packageRoot, '.tmp-autoconnect-empty-settings-document');
...
describe('main.ts auto-connect with empty settings document', () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it('still attempts auto-connect from legacy storage when settings.json exists but lacks saved sections', async () => {
