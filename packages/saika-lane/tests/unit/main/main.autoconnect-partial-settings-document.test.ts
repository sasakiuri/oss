...
const userDataPath = join(packageRoot, '.tmp-autoconnect-partial-settings-document');
...
describe('main.ts auto-connect with partial settings document', () => {
  beforeEach(() => {
    vi.resetModules();
    ...
    writeFileSync(
      ...
      JSON.stringify({ connection: { portName: 'COM9' } }, null, 2),
      'utf8',
    );
  ...
  it('still attempts auto-connect from legacy storage when the persisted connection section is incomplete', async () => {
