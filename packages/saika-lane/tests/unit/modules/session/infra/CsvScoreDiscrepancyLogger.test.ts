...
describe('CsvScoreDiscrepancyLogger', () => {
  ...
  it('creates directory if not exists', () => {
    ...
    expect(fs.mkdirSync).toHaveBeenCalledWith('/tmp/test-logs', { recursive: true, mode: 0o700 });
  });
  ...
  it('uses custom logsDir via constructor parameter', () => {
    ...
    expect(fs.mkdirSync).toHaveBeenCalledWith(customDir, { recursive: true, mode: 0o700 });
    expect(fs.appendFileSync).toHaveBeenCalledWith(expect.stringContaining(customDir), expect.any(String));
  });
});
