...
/** An already-authorized firing step; KEEP_RECORDED_SERIES deliberately has no transport command. */
...
export const StartQualificationRecoveryCommandSchema = CommandBaseSchema.extend({
  runId: z.string().uuid(),
  decisionId: z.string().uuid(),
  interruptionId: z.string().uuid(),
  stageIndex: z.number().int().nonnegative(),
...
export const CancelQualificationRecoveryCommandSchema = CommandBaseSchema.extend({
  runId: z.string().uuid(),
  reason: z.string().trim().min(1).max(500),
});

export const ApplyQualificationRecoveryCommandSchema = CommandBaseSchema.extend({
  runId: z.string().uuid(),
...
export const SettleQualificationRecoveryCommandSchema = CommandBaseSchema.extend({
  decisionId: z.string().uuid(),
  interruptionId: z.string().uuid(),
  stageIndex: z.number().int().nonnegative(),
  seriesIndex: z.number().int().nonnegative(),
