...
import { EliminatePlayerHandler } from '@/main/modules/lane-control/commands/EliminatePlayerHandler';
...
describe('EliminatePlayerHandler', () => {
  let mockRepository: ILaneControlRepository;
  let mockEventBus: IEventBus;
  let handler: EliminatePlayerHandler;
  ...
  beforeEach(() => {
    ...
    handler = new EliminatePlayerHandler(mockRepository, mockEventBus);
  });

  it('should eliminate player with rank and save', async () => {
    const lane = createIdleLaneWithPlayer();
    expect(lane.eliminated).toBe(false);
    expect(lane.eliminationRank).toBeNull();

    vi.mocked(mockRepository.findById).mockReturnValue(lane);

    await handler.execute({ laneId: 'lane-1', rank: 5 });

    expect(mockRepository.save).toHaveBeenCalledTimes(1);
    const savedLane = vi.mocked(mockRepository.save).mock.calls[0]![0];
    expect(savedLane.eliminated).toBe(true);
    expect(savedLane.eliminationRank).toBe(5);
  ...
  it('should return early when lane not found', async () => {
    ...
    await handler.execute({ laneId: 'lane-missing', rank: 3 });
